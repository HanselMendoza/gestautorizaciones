--------------------------------------------------------
--  DDL for Procedure P_VALIDATECOBERTURA
--------------------------------------------------------

  CREATE OR REPLACE PROCEDURE "AUTORIZACIONES"."P_VALIDATECOBERTURA" (
    ...
    IF a%found THEN
        ...
        THEN
          ...
          -- Procedimiento que valida si la cobertura es un estudio a Repeticion  Miguel A.Carrion 06/09/2021
            vestudio_repeticion := f_busca_cob_estudio_repeticion(fonos_row.asegurado, fonos_row.dependiente, fonos_row.compania, fonos_row.
            ramo, fonos_row.secuencial,
                                                                 fonos_row.tip_ser, nvl(fonos_row.tip_cob, v_intip), nvl(fonos_row.cobertura,
                                                                 v_incob), vusuario);
            ...
            IF nvl(vestudio_repeticion, 'N') = 'S' THEN
                ...
                BEGIN
                    ...
                    IF p_error = 0 THEN
                        ...
                        error := dbaper.f_validar_ciclo_cobertura_rep(var_estatus_can, var_tip_a_uso, fonos_row.asegurado, fonos_row.

                        dependiente, p_servicio,
                                                                     p_tip_cob, nvl(fonos_row.cobertura, v_incob), fecha_dia, p_fec_ver,
                                                                     p_fec_fin, p_cantidad,
                                                                     fonos_row.compania);
         ...
         ---Miguel A. Carrion 14/01/2021 Se agrego condicion ya que los procesos busca la fecha de version real del afiliado cuando es basico o voluntario.
            IF fonos_row.plan = dbaper.busca_parametro('PLAN_PBS', fonos_row.compania) OR v_inser = dbaper.busca_parametro('TIP_SERV_CONS_MEDI_0',
            fonos_row.compania) THEN
                var_fec_ini := fdp_fecver_ac(fonos_row.compania, --:PRE_CERTIF.COM_POL,
                 ...
                 sysdate/*:PRE_CERTIF.FEC_ING*/, fonos_row.asegurado,
                                            fonos_row.dependiente);
            ELSIF v_inser = dbaper.busca_parametro('GMM', fonos_row.compania) THEN
                ...
                var_fec_ini := fdp_fecver(fonos_row.compania, fonos_row.ramo, fonos_row.secuencial, sysdate, fonos_row.asegurado,
                                         fonos_row.dependiente, '');
            END IF;
          ...
          --Enfoco mcarrion 12/02/2019
            ...
            THEN
                ...
                CLOSE nuevo;
                IF
                    v_prov_capitado = pkg_const.c_activo
          ...
          --  IF VAR_CODE IS NULL OR VAR_CODE <> 2 THEN
            IF nvl(cod_ase, 0) <> 0 THEN
                ...
                --Solo debe funcionar para las polizas voluntarias
                  ...
                  ---MCARRION 26/06/2019
                            ...
                            IF (
                                ...
                                v_inser = dbaper.busca_parametro('TIP_SERV_CONS_MEDI_0', fonos_row.compania)
                    ...
                    /* Si el servicio es Alto Costo entonces busca la fecha de version del afiliado
                       y la gradualidad segun las cotizaciones o aporte que tenga el afiliado
                        Miguel A. Carrion 14/01/2021  */
                                IF v_inser = dbaper.busca_parametro('TIP_SERV_CONS_MEDI_0', fonos_row.compania) THEN
                                    f_fec_ver := dbaper.fdp_fecver_ac(v_compania_pbs, v_ramo_pbs, v_sec_pbs, fecha_dia, fonos_row.asegurado,
              ...
              --</jdeveaux>
                    ...
                    IF
                        v_inser = dbaper.busca_parametro('TIP_SERV_CONS_MEDI_0', fonos_row.compania)
                        AND fonos_row.ramo = dbaper.busca_parametro('BASICO', fonos_row.compania)
                    THEN
              ...
              -- Fin Miguel A. Carrion 06/05/2021
                ...
                --FONOS_ROW.PLAN
                        ...
                        IF c_valida_plan_excento%notfound THEN
                            IF NOT paq_matriz_validaciones.f_poliza_exento_restriccion(
                                            fonos_row.compania, fonos_row.ramo, 
                                            fonos_row.secuencial, fonos_row.plan, 
                                            fonos_row.tip_ser,fonos_row.tip_cob, 
                                            fonos_row.cobertura,
                                            fonos_row.tip_rec,
                                            fonos_row.afiliado,
                                            vusuario) THEN
                                ori_flag := paq_matriz_validaciones.busca_origen_cob(fonos_row.tip_ser, 
                                            ...
                                            vusuario, fonos_row.ramo, fonos_row.compania);
                  ...
                  /* --------------------------------------------------------------------- */
                    ...
                    /* --------------------------------------------------------- */
                                ...
                                ano_rec, to_char(fecha_dia, 'YYYY')), --Se le agreo el nvl Para en caso que el ano llegue null Tome el ano de la fecha del dia Miguel A. Carrion 18/08/2021 FCCM
                                 ...
                                 fonos_row.sec_rec, fonos_row.tip_rec, fonos_row.afiliado, var_tip_a_uso,
                                                                                                cod_ase, cod_dep, fonos_row.tip_ser, fonos_row.
                            ...
                            --
                              ...
                              --
                               ...
                               /* Si el servicio es GMM le asgina el monto correspondiente segun su limite   Miguel A. Carrion 24/08/2020   */
                                                        IF
                                                            v_inser = dbaper.busca_parametro('GMM', fonos_row.compania)
                                ...
                                --
                                  ...
                                  ----MCARRION COMENTADO PARA PROBAR
                                    ...
                                    --
                                                                ...
                                                                ELSIF ( nvl(v_simultaneo, 'N') = 'S' OR fonos_row.plan = 230 ) THEN
                                     --
                                                                    error1 := true;
                                    --
                                      ...
                                      /***************************************************/
                                        ...
                                        /* --------------------------------------------- */
                                            ...
                                            ---- FIN CAPTURA ERROR                                                          
                                                                    ...
                                                                    ELSE
                                                                        var_code := 2;
                                                                        dbms_output.put_line('VAR_CODE_1 :  ' || var_code);
                                         ---- CAPTURA ERROR
                                                                        INSERT INTO dbaper.log_pkg_infox_htpa (
                                                                            proceso,
                                                                            validacion,
                                            ...
                                            ---- FIN CAPTURA ERROR                               
                                                                ...
                                                                ELSE
                                                                    var_code := 2;
                                                                    dbms_output.put_line('VAR_CODE_2 :  ' || var_code);
                                       ---- CAPTURA ERROR
                                                                    INSERT INTO dbaper.log_pkg_infox_htpa (
                                                                        proceso,
                                                                        validacion,
                                            ...
                                            ---- FIN CAPTURA ERROR   
                                                            ...
                                                            ELSE
                                                                var_code := 2;
                                                                dbms_output.put_line('VAR_CODE_3 :  ' || var_code);
                                   ---- CAPTURA ERROR
                                                                INSERT INTO dbaper.log_pkg_infox_htpa (
                                                                    proceso,
                                                                    validacion,
                                 ...
                                 --
                                  ...
                                  --
                                                            END IF;
                                                            dbms_output.put_line('VAR_CODE_4 :  ' || var_code);
                                            ---- CAPTURA ERROR
                                                            INSERT INTO dbaper.log_pkg_infox_htpa (
                                                                proceso,
                                                                validacion,
                                ...
                                -- del plan tipo de cobertura paralelo
                                                        var_code := 2;
                                                        dbms_output.put_line('VAR_CODE_5 :  ' || var_code);
                                /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                                  (PROCESO, VALIDACION, CODIGO_ERROR)
                                VALUES
                                  ('VALIDATECOBERTURA', NULL, 6);*/
                                                ...
                                                ELSE
                                                    var_code := 2;
                                                    dbms_output.put_line('VAR_CODE_6 :  ' || var_code);
                              /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                                (PROCESO, VALIDACION, CODIGO_ERROR)
                              VALUES
                                ('VALIDATECOBERTURA', NULL, 7);*/
                                            ...
                                            ELSE
                                                var_code := 2;
                                                dbms_output.put_line('VAR_CODE_7 :  ' || var_code);
                            /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                              (PROCESO, VALIDACION, CODIGO_ERROR)
                            VALUES
                              ('VALIDATECOBERTURA', NULL, 8);*/
                                        ...
                                        ELSE
                                            var_code := 2;
                                            dbms_output.put_line('VAR_CODE_8 :  ' || var_code);
                          /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                            (PROCESO, VALIDACION, CODIGO_ERROR)
                          VALUES
                            ('VALIDATECOBERTURA', NULL, 9);*/
                                    ...
                                    ELSE
                                        var_code := 2;
                                        dbms_output.put_line('VAR_CODE_9 :  ' || var_code);
                        /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                          (PROCESO, VALIDACION, CODIGO_ERROR)
                        VALUES
                          ('VALIDATECOBERTURA', NULL, 10);*/
                                ...
                                ELSE
                                    var_code := 2;
                                    dbms_output.put_line('VAR_CODE_10 :  ' || var_code);
                      /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                        (PROCESO, VALIDACION, CODIGO_ERROR)
                      VALUES
                        ('VALIDATECOBERTURA', NULL, 11);*/
                            ...
                            ELSE
                                var_code := 2;
                                dbms_output.put_line('VAR_CODE_11 :  ' || var_code);
                    /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                      (PROCESO, VALIDACION, CODIGO_ERROR)
                    VALUES
                      ('VALIDATECOBERTURA', ERROR, 12);*/
                        ...
                        ELSE
                            var_code := 2;
                            dbms_output.put_line('VAR_CODE_12 :  ' || var_code);
                  /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
                    (PROCESO, VALIDACION, CODIGO_ERROR)
                  VALUES
                    ('VALIDATECOBERTURA', NULL, 13);*/
                        END IF;
        ...
        IF fonos_row.tip_ser <> 3 THEN
          ...
          -- para verificar si el afiliado tiene una marca de suspencion del servicio de salud
            ...
            IF mfraude = 'S' THEN
                ...
                dbms_output.put_line('VAR_CODE_14 :  ' || var_code);
            /*INSERT INTO DBAPER.LOG_PKG_INFOX_HTPA
              (PROCESO, VALIDACION, CODIGO_ERROR)
            VALUES
